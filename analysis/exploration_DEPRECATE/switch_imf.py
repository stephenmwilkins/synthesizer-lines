...
model_families = [
    [
        ('bpass-2.2.1-bin_bpl-0.1,1.0,300.0-1.3,2.0_cloudy-c23.01-fixed', r'$\alpha=2.0$'),
        ('bpass-2.2.1-bin_bpl-0.1,1.0,300.0-1.3,2.35_cloudy-c23.01-fixed', r'$\alpha=2.35$'),
        ('bpass-2.2.1-bin_bpl-0.1,1.0,300.0-1.3,2.7_cloudy-c23.01-fixed', r'$\alpha=2.7$'),
        ('bpass-2.2.1-bin_chabrier03-0.1,100.0_cloudy-c23.01-fixed', r'$m_{up}=100$'),
        # ('bpass-2.2.1-sin_chabrier03-0.1,300.0_cloudy-c23.01-fixed', r'single stars'), 
     ],
]
...
for model_family, family_name in zip(model_families, family_names):
    ...
    for (model, label), ls in zip(model_family, model_line_styles):
        ...
        for line_name, ax in zip(line_names, axes.flatten()):
            ...
            for iz, metallicity in enumerate(grid.metallicity):
                ...
                dline = dgrid.get_line((dia, iz), line_name)
    ...
    fig.savefig(f'figs/ratios-imf.pdf')
    fig.clf()
